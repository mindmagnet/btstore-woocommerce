...
/**
 * WC_MindMagnet_BTpay_Standard class.
 *
 * @extends WC_Payment_Gateway
 */
class WC_Gateway_MindMagnet_BTpay_Standard extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->id = strtolower('MindMagnet_BTPay_Standard');
        $this->method_title = __('BTPay Standard', 'wc_mindmagnet');
        ...
        //var_dump('standard',$_SERVER);
        $options = get_option('woocommerce_MindMagnet_BTPay_Star_settings', array());
        if (!empty($_POST) && strstr($_GET['section'], 'standard')) {
            $options = get_option('woocommerce_MindMagnet_BTPay_Star_settings', array());

            $this->settings['payment_btpay_api_test'] = $options['payment_btpay_api_test'] = $_POST['woocommerce_mindmagnet_btpay_standard_payment_btpay_api_test'];
            $this->settings['payment_btpay_api_debug'] = $options['payment_btpay_api_debug'] = $_POST['woocommerce_mindmagnet_btpay_standard_payment_btpay_api_debug'];
            $this->settings['payment_btpay_api_merchant_name'] = $options['payment_btpay_api_merchant_name'] = $_POST['woocommerce_mindmagnet_btpay_standard_payment_btpay_api_merchant_name'];
            $this->settings['payment_btpay_api_merchant_url'] = $options['payment_btpay_api_merchant_url'] = $_POST['woocommerce_mindmagnet_btpay_standard_payment_btpay_api_merchant_url'];
            $this->settings['payment_btpay_api_merchant_email'] = $options['payment_btpay_api_merchant_email'] = $_POST['woocommerce_mindmagnet_btpay_standard_payment_btpay_api_merchant_email'];
            $this->settings['payment_btpay_api_terminal'] = $options['payment_btpay_api_terminal'] = $_POST['woocommerce_mindmagnet_btpay_standard_payment_btpay_api_terminal'];
            $this->settings['payment_btpay_api_encryption_key'] = $options['payment_btpay_api_encryption_key'] = $_POST['woocommerce_mindmagnet_btpay_standard_payment_btpay_api_encryption_key'];

            update_option('woocommerce_mindmagnet_btpay_standard_settings', $this->settings);
            update_option('woocommerce_mindmagnet_btpay_star_settings', $options);
        }
	    $this->testmode                = 'yes' === $this->get_option( 'payment_btpay_api_test' );
        ...
        $standard = true;
//        if ($this->get_option('payment_btpay_standard_min_order_total') != '') {
//            if (WC()->cart->total < $this->get_option('payment_btpay_standard_min_order_total'))
//                $standard = false;
//        }
//
//        if ($this->get_option('payment_btpay_standard_max_order_total') != '') {
//            if (WC()->cart->total > $this->get_option('payment_btpay_standard_max_order_total'))
//                $standard = false;
//        }

        if ($this->enabled == 'yes' && $standard == true)
            $this->enabled = 'yes';
        else
            $this->enabled = 'no';
        ...
        if (isset($_GET) && $_GET != null) {
            ...
            if ($this->get_option('payment_btpay_api_debug') == '1' && $p != null && $_SERVER['HTTP_REFERER'] != 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] && $meta['_payment_method']['0'] == 'MindMagnet_BTPay_Standard') {
                $settings = PHP_EOL . '  Gateway Response' . PHP_EOL . '';
                foreach ($p as $key => $value) {
                    $settings .= '      ' . $key . ' = ' . $value . PHP_EOL;
                }
                $wc_log->add('btpay-standard-log', 'Gateway response received.' . $settings);
            }
            ...
            if ($_SERVER['HTTP_REFERER'] != $link && $_SERVER['HTTP_REFERER'] != $link . '#api') {
                ...
                if ($log != null) {
                    ...
                    if ($this->get_option('payment_btpay_api_debug') == '1' && $meta['_payment_method']['0'] == 'MindMagnet_BTPay_Standard') {
                        $wc_log->add('btpay-standard-log', 'Transaction saved to DB.');
                    }
                }

                if ($bt_gatewayresponse->isValid($this->get_option('payment_btpay_api_encryption_key'))) {

                    if ($p['ACTION'] == '0') {
                        if (array_key_exists('APPROVAL', $p)) {
                            if ($bt_gatewayresponse->isAuthorized()) {
                                $order->update_status($this->get_option('payment_btpay_standard_order_status'), $log . PHP_EOL . PHP_EOL);
                                if ($this->get_option('payment_btpay_api_debug') == '1' && $meta['_payment_method']['0'] == 'MindMagnet_BTPay_Standard') {
                                    $wc_log->add('btpay-standard-log', 'Response is valid. Order status updated - ' . $this->get_option('payment_btpay_standard_order_status'));
                                }
                            }
                        } elseif ($p['TRTYPE'] == '21') {
                            if ($bt_gatewayresponse->isCaptured()) {
                                ...
                                if ($this->get_option('payment_btpay_api_debug') == '1' && $meta['_payment_method']['0'] == 'MindMagnet_BTPay_Standard') {
                                    $wc_log->add('btpay-standard-log', 'Response is valid. Order status updated - ' . 'completed');
                                }
                            }
                        } elseif ($p['TRTYPE'] == '24') {
                            if ($bt_gatewayresponse->isVoided()) {
                                if ($order->status == 'completed') {
                                    ...
                                    if ($this->get_option('payment_btpay_api_debug') == '1' && $meta['_payment_method']['0'] == 'MindMagnet_BTPay_Standard') {
                                        $wc_log->add('btpay-standard-log', 'Response is valid. Order status updated - ' . 'refunded');
                                    }
                                } else {
                                    $order->cancel_order($log . PHP_EOL . PHP_EOL);
                                    if ($this->get_option('payment_btpay_api_debug') == '1' && $meta['_payment_method']['0'] == 'MindMagnet_BTPay_Standard') {
                                        $wc_log->add('btpay-standard-log', 'Response is valid. Order status updated - ' . 'cancelled');
                                    }
                                }
                            }
                        } elseif ($p['TRTYPE'] == '25') {
                            if ($bt_gatewayresponse->isVoided()) {
                                ...
                                if ($this->get_option('payment_btpay_api_debug') == '1' && $meta['_payment_method']['0'] == 'MindMagnet_BTPay_Standard') {
                                    $wc_log->add('btpay-standard-log', 'Response is valid. Order status updated - ' . 'processing');
                                }
                            }
                        }
                    } else {
                        ...
                        if ($this->get_option('payment_btpay_api_debug') == '1' && $meta['_payment_method']['0'] == 'MindMagnet_BTPay_Standard') {
                            $wc_log->add('btpay-standard-log', 'Response is valid. Order status updated - ' . 'failed');
                        }
                    }
                    ...
                    if ($this->get_option('payment_btpay_api_debug') == '1' && $meta['_payment_method']['0'] == 'MindMagnet_BTPay_Standard') {
                        $wc_log->add('btpay-standard-log', 'Response is not valid.');
                    }
                }
            }
        }
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'label' => __('Enable BTpay Standard', 'wc_mindmagnet'),
                'type' => 'checkbox',
                'description' => '',
                'default' => 'no'
            ),
            'title' => array(
                ...
                'default' => __('Card de credit (prin Banca Transilvania)', 'wc_mindmagnet'),
                'desc_tip' => true
            ),
            'payment_btpay_standard_order_status' => array(
            ...
            'payment_btpay_standard_min_order_total' => array(
            ...
            'payment_btpay_standard_max_order_total' => array(
...
//                print_R($link);exit;
                ...
                if ($this->get_option('payment_btpay_api_debug') == '1' && $meta['_payment_method']['0'] == 'MindMagnet_BTPay_Standard') {
                    ...
                    $wc_log->add('btpay-standard-log', 'Checkout finished and data sent to BT.' . PHP_EOL . $settings);
                }
                $woocommerce->cart->empty_cart();
                die($bt_paygate->renderForm($params, false, true));
            } else
                throw new Exception('Invalid1/incomplete API configuration', 1);
        }
    }
